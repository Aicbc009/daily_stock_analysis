...
- 安装来源：默认固定到已验证的 AlphaSift 适配层 commit：`ALPHASIFT_INSTALL_SPEC=git+https://github.com/ZhuLinsen/alphasift.git@b2ca66dd47001b9a09890cfe21c2b18c7219ccf5`。
- 自动安装：Web 端开启时会调用后端 `/api/v1/alphasift/install`；该接口要求 `ADMIN_AUTH_ENABLED=true` 且当前请求持有有效管理员会话，并且只允许默认受信任来源自动安装。本地路径、wheel、其他来源或未开启管理员认证的部署，需要用户先手动安装到当前 Python 环境。
...
- `/api/v1/alphasift/install`：只在启用后执行；要求有效管理员会话，只允许默认受信任安装来源自动安装；安装后校验适配层。
...
- 未开启管理员认证或没有有效管理员会话时，自动安装返回 `403 alphasift_install_auth_required` 或 `401 alphasift_install_unauthorized`。
